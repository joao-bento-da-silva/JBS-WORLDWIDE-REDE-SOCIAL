...
# ==============================================
# INTELIGÊNCIA ARTIFICIAL — COM BASE DE CONHECIMENTO APRENDIZ
# ==============================================
...
CONHECIMENTO_PADRAO = {
    ...
    "quem é você": "Eu sou a IA da JNB TECNOLOGIA, criada por João Bento da Silva.",
    "jogo de cartas": "🃏 Regra: Y→Y, A→Z, Z→A, B→X, X→B, C→G, G→C, D→F, F→D, E→E.",
    "jogo bentinho": "🎮 Regra: 0↔0, 1↔9, 2↔8, 3↔7, 4↔6, 5↔5, 6↔4, 7↔3, 8↔2, 9↔1.",
    "dna": "🧬 Cada usuário tem sua chave única. Use para criptografar e salvar como .bnj!",
    ...
    if request.method == "POST":
        ...
        if pergunta and resposta:
            try:
                ...
                msg = "⚠️ Já tenho essa pergunta cadastrada!"
        else:
            msg = "❌ Preencha pergunta e resposta!"
    return render_template_string('''<!DOCTYPE html><html lang="pt-br"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>📚 Ensinar IA</title><script src="https://cdn.tailwindcss.com"></script><style>body{background:linear-gradient(180deg,#0f172a,#1e293b);color:#e2e8f0;min-height:100vh;}</style></head><body class="p-6 max-w-2xl mx-auto"><a href="/plataforma" class="text-yellow-500 mb-4 inline-block">← Voltar</a><h1 class="text-3xl font-bold text-yellow-500 mb-6">📚 ENSINAR A INTELIGÊNCIA ARTIFICIAL</h1>{% if msg %}<div class="p-4 rounded-lg mb-6 font-bold {{'bg-green-900/50 text-green-400' if '✅' in msg else 'bg-yellow-900/50 text-yellow-400'}}">{{msg}}</div>{% endif %}<form method="POST" class="bg-gray-800 p-6 rounded-lg border border-yellow-500/30"><label class="block mb-2 font-bold">Pergunta ou palavra-chave:</label><input type="text" name="pergunta" class="w-full p-3 bg-gray-900 border border-gray-700 rounded-lg mb-4 text-white" placeholder="Ex: capital do Brasil" required><label class="block mb-2 font-bold">Resposta que a IA deve aprender:</label><textarea name="resposta" class="w-full p-3 bg-gray-900 border border-gray-700 rounded-lg mb-4 text-white" rows="4" placeholder="Digite a resposta correta..." required></textarea><button type="submit" class="bg-yellow-600 text-black font-bold px-6 py-3 rounded-lg">💾 Salvar na Base de Conhecimento</button></form><p class="mt-6 text-gray-400 text-sm">Cada novo ensinamento fica salvo para todos os usuários. A IA aprende e melhora cada dia!</p></body></html>''', msg=msg)
...
def descriptografar(dados_b64, chave):
    try:
        ...
        return "ERRO: Chave inválida ou dados corrompidos!"
...
def dna_criptografar():
    ...
    chave = c.fetchone()[0]
    conn.close()
    cripto = criptografar(texto, chave)
    return cripto
...
# ==============================================
# JOGO BENTINHO — SEGREDO DOS NÚMEROS (REGRA DEFINITIVA)
# ==============================================
...
def painel_dono():
    ...
    conn.close()
    return render_template_string(f'''<!DOCTYPE html><html lang="pt-br"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>⚙️ Painel do Dono</title><script src="https://cdn.tailwindcss.com"></script><style>body{{background:linear-gradient(180deg,#0f172a,#1e293b);color:#e2e8f0;min-height:100vh;}}</style></head><body class="p-6 max-w-4xl mx-auto"><h1 class="text-3xl font-bold text-yellow-500 mb-6">⚙️ PAINEL DO DONO</h1><a href="/plataforma" class="text-yellow-500 mb-4 inline-block">← Voltar</a><div class="grid grid-cols-3 gap-4"><div class="bg-gray-800 p-4 rounded-lg border border-yellow-500/30"><p class="text-gray-400">Total de Usuários</p><p class="text-2xl font-bold text-yellow-500">{total_usuarios}</p></div><div class="bg-gray-800 p-4 rounded-lg border border-yellow-500/30"><p class="text-gray-400">Total de Postagens</p><p class="text-2xl font-bold text-yellow-500">{total_postagens}</p></div><div class="bg-gray-800 p-4 rounded-lg border border-yellow-500/30"><p class="text-gray-400">Conhecimento da IA</p><p class="text-2xl font-bold text-yellow-500">{total_conhecimento}</p></div></div></body></html>''')
