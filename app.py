...
# VISUAL INTACTO • TODOS OS SERVIÇOS ATIVOS
# ==================================================
    ...
    <head>
        ...
        <style>
            *{box-sizing:border-box;margin:0;padding:0;font-family:Arial,sans-serif}
            ...
            .botao{display:inline-block;margin:15px;padding:15px 30px;background:#1e293b;border:3px solid #10b981;border-radius:15px;color:white;text-decoration:none;font-size:18px}
            .global{color:#3b82f6;font-weight:bold;margin-top:10px}
        </style>
    </head>
    <body>
...
def cadastro():
    ...
    <html><body style="background:#0f172a;color:white;text-align:center;padding:20px;">
        ...
        <form method="POST">
            ...
            <button type="submit" style="padding:12px 40px;background:#10b981;border:none;border-radius:8px;color:white;margin-top:10px;">Cadastrar</button>
        </form>
    ...
    <head>
        ...
        <style>
            *{{box-sizing:border-box;margin:0;padding:0;font-family:Arial,sans-serif}}
            ...
            .servico{{display:block;max-width:420px;margin:12px auto;padding:18px;background:#1e293b;border:3px solid #10b981;border-radius:15px;color:white;text-decoration:none;font-size:20px;text-align:left;padding-left:30px}}
            .servico.destaque{{border-color:#f59e0b}}
            .sair{{color:#ef4444;margin-top:30px;text-decoration:none;font-size:18px}}
        </style>
    </head>
    <body>
        <h1>JNB TECNOLOGIA 🌍</h1>
        <p class="sub">PLATAFORMA GLOBAL FUNCIONAL</p>
        ...
        <a href="/bnj_servico" class="servico destaque">🧬🔢 REGISTRO BNJ • FERRAMENTA</a>
...
def documentos():
    ...
    <body>
        ...
        <p>Funcionalidade ativa e pronta para usar ✅</p>
    ...
    <head>
        ...
        <style>
            *{{box-sizing:border-box;margin:0;padding:0;font-family:Arial,sans-serif}}
            ...
            textarea{{width:100%;height:80px;padding:10px;border-radius:8px;border:none;margin:10px 0}}
            button{{padding:12px 30px;background:#f59e0b;border:none;border-radius:8px;color:#000;font-weight:bold}}
        </style>
    </head>
    <body>
        ...
        {''.join([f'<div class="post"><div class="nome">{p[0]}</div><div>{p[1]}</div><div style="color:#94a3b8;font-size:12px">{p[2]}</div></div>' for p in posts])}
    ...
    <head>
        ...
        <style>
            *{{box-sizing:border-box;margin:0;padding:0;font-family:Arial,sans-serif}}
            ...
            .produto{{background:#1e293b;padding:15px;border-radius:10px;margin:10px auto;max-width:400px}}
            .preco{{color:#10b981;font-weight:bold;font-size:20px}}
        </style>
    </head>
    <body>
        <h1>🏆 LOJA JNB</h1>
        {''.join([f'<div class="produto"><h3>{p[0]}</h3><p>{p[1]}</p><div class="preco">R$ {p[2]:.2f}</div></div>' for p in produtos])}
        <br><a href="/painel" style="color:#3b82f6;display:block;text-align:center">← Voltar</a>
    </body>
    </html>
    """)
...
def bnj_servico():
    ...
    if request.method == "POST":
        ...
        if plano != "gratuito":
            if acao == "varrer":
                resultado = """
✅ VARREDURA CONCLUÍDA 🧬🔢<br>
• Binário: OK ✅<br>
• Hexadecimal: OK ✅<br>
• Padrão BNJ: OK ✅
                """
                mensagem = "SISTEMA LIMPO"
                cor = "#10b981"
            elif acao == "reparar":
                resultado = """
🔧 REPARO FINALIZADO ✅<br>
• Erros: 0%<br>
• Velocidade: +20% ✅
                """
                mensagem = "SISTEMA FORTE"
                cor = "#3b82f6"
            elif acao == "chave":
                chave = ''.join(random.choice("TCGA0123456789ABCDEF") for _ in range(64))
                resultado = f"🔑 CHAVE: <b>{chave[:20]}...</b>"
                mensagem = "PROTEÇÃO GARANTIDA"
                cor = "#f59e0b"
        else:
            mensagem = "⚠️ Acesse plano premium para todas funções!"
            cor = "#ef4444"
    ...
    <head>
        ...
        <style>
            *{{margin:0;padding:0;box-sizing:border-box;font-family:Arial,sans-serif}}
            body{{background:#0f172a;color:white;padding:20px}}
            .card{{max-width:500px;margin:auto;background:#1e293b;padding:25px;border-radius:15px;border:2px solid {cor}}}
            button{{width:100%;padding:12px;margin:8px 0;background:#3b82f6;border:none;border-radius:8px;color:white;font-weight:bold}}
            .destaque{{background:#f59e0b}}
            .resultado{{padding:15px;border-radius:8px;text-align:center;margin-top:15px;border:2px solid {cor};color:{cor}}}
        </style>
    </head>
    <body>
        <div class="card">
            <h2 style="text-align:center;color:#f59e0b">🧬🔢 REGISTRO BNJ</h2>
            <p style="color:#cbd5e1;text-align:center">FERRAMENTA GLOBAL</p>
            {f'<div class="resultado">{mensagem}<br><br>{resultado}</div>' if mensagem else ''}
            <form method="POST">
                <button type="submit" name="acao" value="varrer">🔍 VARREDURA</button>
                <button type="submit" name="acao" value="reparar">🔧 REPARAR</button>
                <button type="submit" name="acao" value="chave" class="destaque">🔑 GERAR CHAVE</button>
            </form>
        </div>
        <br><a href="/painel" style="color:#3b82f6;text-align:center;display:block;">← Voltar</a>
    </body>
    </html>
    """)
    ...
    <head>
        ...
        <style>
            *{{box-sizing:border-box;margin:0;padding:0;font-family:Arial,sans-serif}}
            body{{background:#0f172a;color:white;text-align:center;padding:20px}}
            h1{{color:#f59e0b}}
            .opcao{{display:block;margin:15px auto;padding:15px;background:#1e293b;border:2px solid #10b981;border-radius:10px;width:250px;cursor:pointer;font-size:18px}}
            .mensagem{{margin:20px;padding:15px;border-radius:10px;border:2px solid {cor};color:{cor}}}
        </style>
    </head>
    <body>
        <h1>🎮 JOGO DOS PARES</h1>
        <p>Encontre o par correto:</p>
        ...
        <a href="/jogo_pares" style="color:#3b82f6;">JOGAR NOVAMENTE</a>
        <br><a href="/painel" style="color:#3b82f6;margin-top:10px">← Voltar</a>
    </body>
    </html>
    """)

# ==================================================
# ✅ AQUI ESTÁ A LINHA QUE VOCÊ QUERIA — FECHADA E CORRETA
# ==================================================
